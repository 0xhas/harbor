...
export function WatchNowCard({ watching }: { watching?: ProfileWatching }) {
  ...
  const pos = Math.min(Math.max(0, watching.positionSec ?? 0), dur > 0 ? dur : Infinity);
  const hasTime = dur > 0;
  const pct = hasTime ? Math.min(100, Math.max(0, (pos / dur) * 100)) : 0;
  const label = isParty ? t("Watch party") : watching.paused ? t("Paused") : t("Watch now");
  const sub =
  ...
  return (
    <section className="rounded-[14px] bg-surface p-4 ring-1 ring-edge-soft">
